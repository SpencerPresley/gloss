...
The `docq` console script ([`src/docq/cli.py`](../src/docq/cli.py)) has seven subcommands:
`retrieve`, `show`, and `embed` (query-side, stdlib packages only), `build`,
`enrich-questions`, `eval`, and the read-only `config` inspector. A subcommand is
**required** — running `docq` with no args exits non-zero.

```
docq [-h] [--config FILE] {config,retrieve,show,embed,build,enrich-questions,eval} ...
```

`retrieve`, `show`, and `embed` import only stdlib modules (`store` / `vectors`) — though `embed` and the hybrid retrieve modes additionally need a **running local Ollama** (a service, not a package). `build` and `eval` lazily import their build-only deps inside the command function, so `retrieve` never pulls them in. Run query-side commands with plain `uv run docq ...`; run `build`/`eval` with `uv run --extra build docq ...` (eval needs `pyyaml`; see [Errors](#error--exit-behavior)).

## Configuration

Automatic configuration merges these files in order:

1. `$XDG_CONFIG_HOME/docq/config.json`, or `~/.config/docq/config.json` when XDG is unset.
2. `<git-root>/.docq/config.json`.
3. Explicit CLI flags.

Project values override overlapping user values. `--config FILE`, accepted before or after the
subcommand, disables both automatic files; CLI flags still override it. Relative paths in a
project config resolve from the Git root. Run `docq config` to print the loaded sources,
locations considered, and effective values. Unknown keys and wrong value types are errors.

`db` and `ollama-url` are top-level shared settings. Options whose names overlap between commands
live under command objects such as `retrieve.rerank-model`, `embed.model`, and `build.model`.
...
```
docq retrieve [-h] [--db DB] [-k K] [--principle PRINCIPLE] [--type TYPE] [--json]
...
|-----|------|---------|---------|
| `query` | positional, str | *(required)* | Free-text design situation. Tokenized to an FTS5 `MATCH` expr: tokens >2 chars are OR-joined with prefix globs ([store.py:34-42](../src/docq/store.py#L34)). A query with no usable (>2-char) tokens matches nothing. |
| `--db` | str | config | Path to the SQLite/FTS5 corpus db. Overrides the configured top-level `db`; errors if neither exists. |
| `-k` | int | `5` | Max number of hits to return. |
| `--principle` | str, repeatable | `None` | Filter to one or more coarse principle slugs. `action="append"` — pass the flag once per value ([cli.py:51](../src/docq/cli.py#L51)). |
| `--type` | str, repeatable | `None` | Filter to one or more unit types. `action="append"` ([cli.py:52](../src/docq/cli.py#L52)). |
...
| `--rerank` / `--no-rerank` | flag | config or off | LLM-rerank the top candidates (fetches ≥5 even at `-k 1`, returns top-k). **Gated**: when fusion's #1 is dual-backed (lexical #1 + semantic top-5) it is trusted and no model call happens. A malformed reply keeps the original order; transport, authentication, usage, and missing-model failures exit nonzero with context. |
...
Filters are AND-combined across facets, OR-combined within a facet (`u.principle IN (...) AND u.type IN (...)`, [store.py:79-89](../src/docq/store.py#L79)). Filters apply to both channels.

### Output: default (text)

Each hit renders as a one-line citation header followed by the verbatim passage and a trailing blank line ([`_format_hit`, cli.py](../src/docq/cli.py)):
...
```console
$ uv run docq retrieve "deep module hides complexity" --db build/minimax-v2.db -k 1
...
[`docq show <id>`](#show) — never quote a preview line.

```console
$ uv run docq retrieve "callers must call setup in the right order" --db build/minimax-v2.db -k 3 --compact
...
Prints `json.dumps(hits, indent=2)` ([cli.py:24](../src/docq/cli.py#L24)) — a JSON array of objects, one per hit. Keys come straight from the `units` row plus the BM25 `score` ([`search`, store.py:87-96](../src/docq/store.py#L87)):
...
```console
$ uv run docq retrieve "deep module hides complexity" --db build/minimax-v2.db -k 1 --json
[
  {
...
uv run docq retrieve "my class just forwards calls and adds nothing" --db build/minimax-v2.db

# filter to one principle, more results
uv run docq retrieve "callers must call setup in the right order" \
  --db build/minimax-v2.db --principle information-hiding -k 10

# repeat a flag for OR within a facet
uv run docq retrieve "shallow class" --db build/minimax-v2.db \
  --type red_flag --type rationale

# combine facets (AND across, OR within)
uv run docq retrieve "shallow helper manager" --db build/minimax-v2.db \
  --principle deep-modules --type red_flag

# force a mode
uv run docq retrieve "boolean flag for one caller" --db build/minimax-v2.db --mode hybrid
uv run docq retrieve "boolean flag for one caller" --db build/minimax-v2.db --mode lexical
```
...
Print one unit in full by id ([`store.py:get_unit`](../src/docq/store.py), one SELECT,
...
unit into a `vectors` table inside the **same** `.db` file ([`vectors.py:embed_corpus`](../src/docq/vectors.py)).
Stdlib packages only, but needs a running local Ollama serving the embedding model
(`ollama pull embeddinggemma`).

```
docq embed [-h] [--db DB] [--model MODEL] [--ollama-url URL] [--batch BATCH]
```

| Arg | Type | Default | Meaning |
|-----|------|---------|---------|
| `--db` | str | config | Corpus db to embed. Vectors are written into this file; prior vectors are replaced. |
...
single portable file (~7 MB with vectors). **Re-run after every `docq build`** — a
...
Build the corpus db from the source document (parse → segment → enrich → store). Build-only deps are lazily imported ([cli.py:30-33](../src/docq/cli.py#L30)); run with the `build` extra and a reachable Ollama model. Operational details (checkpoints, resume, workers, model A/B, troubleshooting) live in [BUILDS.md](BUILDS.md); the fresh-checkout runbook is in [STARTUP_GUIDE.md](STARTUP_GUIDE.md).

```
docq build [-h] [--chapter CHAPTER] [--model MODEL] [--db DB] [--instance DIR]
            [--resume] [--workers WORKERS] [--build-dir BUILD_DIR]
```

| Arg | Type | Default | Meaning |
|-----|------|---------|---------|
| `--chapter` | str | `None` | Build a single chapter by id. `None` = build the whole book + appendices ([cli.py:57](../src/docq/cli.py#L57), [build.py:80-86](../src/docq/build.py#L80)). |
| `--model` | str | `minimax-m3:cloud` | Ollama model used for enrichment, recorded per-unit in `enrich_model`. |
| `--db` | str | config | Output db path (overwritten on build). |
| `--instance` | str | config | Corpus directory containing `profile.py`, taxonomy, and prompts. Errors if neither flag nor config supplies it. |
| `--resume` | flag | off | Keep existing per-chapter JSONL checkpoints instead of wiping them; re-enriches only failed units. |
| `--workers` | int | `1` | Concurrent enrichment requests per chapter (`1` = serial) ([cli.py:61](../src/docq/cli.py#L61)). |
| `--build-dir` | str | `build` | Root dir for per-chapter JSONL checkpoints. **Use a distinct dir per model** (e.g. `build/minimax`) so concurrent/sequential model builds don't clobber each other's checkpoints ([cli.py:62-64](../src/docq/cli.py#L62)). |

### Example

```console
$ uv run --extra build docq build --chapter 1 \
...
The startup line `chapters=N units=M num_ctx=C model=...` is printed before enrichment ([build.py:101](../src/docq/build.py#L101)). If the largest prompt's estimated token count exceeds the `num_ctx` cap, a `WARNING: largest prompt ~N est tokens exceeds num_ctx cap C; trim situating context or raise the cap` is printed ([build.py:42-43](../src/docq/build.py#L42)).
...
```
docq enrich-questions [-h] [--build-dir BUILD_DIR] [--instance DIR]
                      [--model MODEL] [--workers WORKERS]
```

| Arg | Type | Default | Meaning |
|-----|------|---------|---------|
| `--build-dir` | str | config | Checkpoint root of the build to top up (e.g. `build/minimax-v2`). |
| `--instance` | str | config | Corpus directory containing the question prompt and taxonomy. |
...
```
docq eval [-h] [--db DB] [--cases CASES] [-k K] [-v]
           [--mode {lexical,hybrid,semantic}] [--ollama-url URL]
```

| Arg | Type | Default | Meaning |
|-----|------|---------|---------|
| `--db` | str | config | Corpus db to evaluate against. |
| `--cases` | str | config | YAML file with a `cases:` list. Each case has `query` plus any of `expect_principle` / `expect_section` / `expect_chapter`; a result matching **any** pinned field counts ([evalrun.py:8-23](../src/docq/evalrun.py#L8)). |
...
| `--vs` | choice | — | Second mode to compare against: prints both score lines plus `Δmrr` and a p-value from a paired sign-flip randomization test on per-case reciprocal rank ([evalrun.py:paired_sign_flip](../src/docq/evalrun.py)) — use it before adopting any knob change. |
...
$ uv run --extra build docq eval --db build/minimax-v2.db --mode hybrid --vs lexical
...
A `CHECK` constraint pins the set at the schema level ([store.py:16](../src/docq/store.py#L16)). Confirmed against the built corpus:
...
| Situation | Result |
|-----------|--------|
| Query with no >2-char tokens | `search` returns `[]`; text mode prints `(no matches)`, `--json` prints `[]` ([store.py:77-78](../src/docq/store.py#L77)). |
| `retrieve` / `eval` against a db with no `units_fts` table (e.g. a fresh 0-byte `build/aposd.db`) | `sqlite3.OperationalError: no such table: units_fts` — uncaught traceback ([store.py:93](../src/docq/store.py#L93)). See [BUILDS.md](BUILDS.md) for the 0-byte-db gotcha. |
...
| `retrieve --mode hybrid`/`semantic` against a db with no vectors, or with Ollama down | `SystemExit: docq retrieve --mode …: db has no vectors — run: docq embed …` (or `embedder unreachable at <url>`). Explicit modes fail loudly. |
| `show <id>` where no unit has that id | `SystemExit: docq show: no unit with id=N in <db>` — non-zero exit, message on stderr. |
| `retrieve` (auto mode) when vectors exist but the embedder is down | Falls back to lexical and prints `docq: semantic channel off (…); lexical only` to **stderr**; results still returned. |
...
| `build --chapter X` where `X` isn't detected | `raise SystemExit("chapter 'X' not found by detection/override")` ([build.py:86](../src/docq/build.py#L86)). |
| No database in CLI or config | argparse error naming `--db` and every config path inspected. |
| `--rerank` with Ollama unavailable, unauthenticated, out of usage, or missing a model | Nonzero exit with the model, URL/status, Ollama response, and targeted remediation. Known setup failures do not dump a traceback. |
| No subcommand given | argparse error, non-zero exit (`required=True`, [cli.py:45](../src/docq/cli.py#L45)). |
