...
- **Engine package = `docq`** (working name; see `docs/superpowers/notes/2026-06-10-naming.md`): `src/docq/`, CLI `docq`, imports `docq.*`. The instance stays `corpora/aposd/` and its artifact `aposd.db`. Where v1 snippets below show `aposd.*` / `src/aposd/` for the *engine*, read `docq`.
